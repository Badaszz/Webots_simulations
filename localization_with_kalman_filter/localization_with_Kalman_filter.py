...
## KF function
def Kf_step(x, y, phi, P, u, w, imu_phi, delta_t, Q, R):
    ...
    # Update step
    H = np.array([[0, 0, 1]]) # picks out phi from the matrix
    
    # Innovation - how surprising the prediction is based on the measurement
    innovation = wrap_angle(imu_phi - phi_pred)
    
    # Kalman Gain - how much of the innovation should be beleived
    S = H @ P_pred @ H.T + R
    K = P_pred @ H.T / S
    
    # Update the predicted state using the kalman gain
...
### Tuning the R - IMU error
# aggregate variance of IMU measurements over time with the robot at rest
# samples = []
# for _ in range(500):   # collect 500 readings
    # robot.step(timestep)
    # gyro_values = gyro.getValues()   # [wx, wy, wz]
    # wz = gyro_values[2]              # yaw rate in rad/s
    # samples.append(wz)   # yaw readings
    
# R = np.var(samples)   # variance of yaw while stationary = R
# print(f"R = {R}") 
R_error = 4.6
...
while robot.step(timestep) != -1:
    ...
    gyro_values = gyro.getValues()   # [wx, wy, wz]
    wz = gyro_values[2]              # yaw rate in rad/s
    ## Gyro measurement of orientation 
    gyro_phi =  gyro_phi + (wz * delta_t)  
    gyro_phi  = wrap_angle(gyro_phi)      
    ...
    # KF step
    x, y, phi, P = Kf_step(x, y, phi, P, u, w, gyro_phi, delta_t, Q, R_error)
